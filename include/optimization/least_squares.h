...
class least_squares_migration{
    public:
        ...
        void run(const std::vector<float>& data, optimizers optimizer = optimizers::SIMPLE_GRADIENT, int max_iterations = 250, float tol = 1e-6, int verbosity = 2);
    ...
    private:
        ...
        // Line Search Helper Methods
        float evaluate_objective_at_step(const std::vector<float>& direction, float step_size);
        void vector_add_scaled(std::vector<float>& result, const std::vector<float>& vec1, 
                              const std::vector<float>& vec2, float scale);
